...
> Q. Make a clustering results fiture of the data colored by cluster membership.
...
> Q. Run kmeans again and cluster into 4 groups/clusters and plot the result like we did above?
...
The meain "base" R function for Hierarchical Clustering is called `hclust()`. Here we can't just input our data we need to first calculate a distance matrix (e.g. `dist()`) for our data and use this as input to `hclust()`
```{r}
d <- dist(x)
hc <- hclust(d)
hc
...
To get our cluster "membership" vector (i/e our main clustering result) we can "cut" the tree at a given height or at a height that yields a given "k" group
