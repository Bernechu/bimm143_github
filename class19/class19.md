...
Add some major milstone timepoints to our plot:
...
The full introduction of mandatory wP (whole-cell) Pretussis
immunization in the mid 1940s lead to a dramatic reduction in case
numbers (from over 200,000 to 100s)

The switch to the aP (newer acellular) formalization in 1996 coincides
with a gradual increase in case numbers over the next two decades.

The 2020 lockdowns and social distancing measures implemented to combat
the COVID-19 pandemic lead to a dramatic drop in Pertussis cases, but
case numbers have rebounded in the last few years.
...
The make their data avaliable via JSON format API endpoints - basically
...
> Q. How many wP and aP subjexts are there?
...
sample of it’s type ever assembled.
...
Now let’s join the `ab_titer` with our `meta` table so we have all the
information in one placeabout a given Ab measurement
...
> How many Ab measurements do we have in total?
...
> Q. How many different isotypes (type of Ab) are in the dataset?
...
IgG is crucial for long-term immunity and responding to basterial &
viral infections
...
More advance analysis digging into individual antigen responses over
time:
...
responses for large set of wP (teal color) and aP (read color)
individuals. Levels peak at day 14 and are larger in magnitude for wP
than aP in individuals.
